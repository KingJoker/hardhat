...
describe("Truffle fixtures support", function () {
    describe("Migration detection", function () {
        describe("In a project without migrations", function () {
            helpers_1.useEnvironment("hardhat-project-solc-0.4", plugins_1.HARDHAT_NETWORK_NAME);
            it("Should not detect any", async function () {
                chai_1.assert.isFalse(await fixture_1.hasMigrations(this.env.config.paths));
            });
        });
        describe("In a project with migrations", function () {
            helpers_1.useEnvironment("hardhat-project-with-migrations", plugins_1.HARDHAT_NETWORK_NAME);
            it("Should detect them", async function () {
                chai_1.assert.isTrue(await fixture_1.hasMigrations(this.env.config.paths));
            });
        });
    });
    describe("Fixtures detection", function () {
        describe("In a project without fixture", function () {
            helpers_1.useEnvironment("hardhat-project-solc-0.4", plugins_1.HARDHAT_NETWORK_NAME);
            it("Should not detect any", async function () {
                chai_1.assert.isFalse(await fixture_1.hasTruffleFixture(this.env.config.paths));
            });
        });
        describe("In a project with a js fixture", function () {
            helpers_1.useEnvironment("hardhat-project-with-fixture", plugins_1.HARDHAT_NETWORK_NAME);
            it("Should detect them", async function () {
                chai_1.assert.isTrue(await fixture_1.hasTruffleFixture(this.env.config.paths));
            });
        });
        describe("In a project with a ts fixture", function () {
            helpers_1.useEnvironment("hardhat-project-with-ts-fixture", plugins_1.HARDHAT_NETWORK_NAME);
            it("Should detect them", async function () {
                chai_1.assert.isTrue(await fixture_1.hasTruffleFixture(this.env.config.paths));
            });
        });
    ...
    describe("Fixtures function loading", function () {
        describe("In a project with a js fixture", function () {
            ...
            it("Should load it correctly", async function () {
                const fixture = await fixture_1.getTruffleFixtureFunction(this.env.config.paths);
        ...
        describe("In an invalid fixture", function () {
            helpers_1.useEnvironment("hardhat-project-with-invalid-fixture", plugins_1.HARDHAT_NETWORK_NAME);
            it("Should load it correctly", async function () {
                try {
                    await fixture_1.getTruffleFixtureFunction(this.env.config.paths);
                }
                catch (error) {
    ...
    describe("Fixtures integration test", function () {
        helpers_1.useEnvironment("hardhat-project-solc-0.5", plugins_1.HARDHAT_NETWORK_NAME);
