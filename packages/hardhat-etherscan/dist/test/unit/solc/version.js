...
describe("solc version retrieval unit tests", function () {
    ...
    it("solc version with commit is returned", async () => {
        nock_1.default("https://solc-bin.ethereum.org")
            .get("/bin/list.json")
            .reply(200, {
            releases: {
        ...
        const fullVersion = await version_1.getLongVersion("0.5.1");
    ...
    it("an exception is thrown if there was an error sending request", async () => {
        nock_1.default("https://solc-bin.ethereum.org").get("/bin/list.json").reply(404);
        return version_1.getLongVersion("0.5.1")
